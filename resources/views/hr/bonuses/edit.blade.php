    ...
    <section class="bonus-table">
        <div class="card p-2">
            {{-- <div class="card-header">
                <h5>{{ trans('global.create') }} {{ trans('cruds.bonus.title_singular') }}</h5>
            </div> --}}
            <div class="card-body p-2">
                <form action="{{ route('hr.bonuses.update',$bonus) }}" method="post">
                    @method('PATCH')
                    @csrf
                    <div class="row">
                        <div class="col-xl-4 col-lg-4 col-md-4 col-12 col-sm-12 mb-2">
                            <div class="form-group">
                                ...
                                <input type="text" name="date" id="date" placeholder="ရက်စွဲ ရွေးချယ်ပါ။" value="{{ $bonus->date }}"  class="form-control {{ $errors->has('date') ? 'is-invalid' : '' }}"/>
                                @if($errors->has('date'))
                        ...
                        <div class="col-xl-4 col-lg-4 col-md-4 col-12 col-sm-12 mb-2">
                            <div class="form-group">
                                <label class="required mb-2" for="employee_id">@lang('cruds.employee.title')</label>
                                <select name="employee_id" class="form-control select2 {{ $errors->has('employee_id') ? 'is-invalid' : '' }}">
                                    ...
                                    @foreach ($employees as $id => $name)
                                        <option value="{{ $id }}"{{ $bonus->employee_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-xl-4 col-lg-4 col-md-4 col-12 col-sm-12 mb-2">
                            <div class="form-group">
                                ...
                                <select name="bonus_type" class="form-control select2 {{ $errors->has('bonus_type') ? 'is-invalid' : '' }}">
                                    <option value="" disabled selected>{{ trans('global.please_select') }}</option>
                                    @foreach ($bonus_types as $bonus_type)
                                        <option value="{{ $bonus_type }}"{{ $bonus->bonus_type == $bonus_type ? 'selected' : '' }}>@lang("global.$bonus_type")</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-xl-4 col-lg-4 col-md-4 col-12 col-sm-12 mb-2">
                            <div class="form-group">
                                ...
                                <input type="text" name="amount" id="amount" placeholder="@lang('global.number_placeholder')" value="{{ $bonus->amount }}"  class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}"/>
