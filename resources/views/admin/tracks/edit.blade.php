...
<main id="main" class="main">
    ...
    <form action="{{ route('admin.track.update',$track) }}" method="post">
        @csrf
        @method('PUT')
                            ...
                            <div class="form-group">
                                ...
                                <select name="from" id="from" class="form-control {{ $errors->has('from') ? 'is-invalid' : '' }}">
                                    ...
                                    @foreach ($cities as $id => $name)
                                        <option value="{{ $id }}"{{ $track->from == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('from'))
                        ...
                        <div class="col-xl-6 col-lg-6 col-md-6 col-12 col-sm-12 mb-2">
                            <div class="form-group">
                                ...
                                <select name="to" id="to" class="form-control {{ $errors->has('to') ? 'is-invalid' : '' }}">
                                    <option value="" disabled selected>{{ trans('global.please_select') }}</option>
                                    @foreach ($cities as $id => $name)
                                        <option value="{{ $id }}"{{ $track->to == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('to'))
                                    <div class="invalid-feedback">
                    ...
                    <div class="row">
                        <div class="col-xl-6 col-lg-6 col-md-6 col-12 col-sm-12 mb-2">
                            <div class="form-group">
                                ...
                                <input type="number" name="amount" id="amount" value="{{ $track->amount }}"  class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}"/>
                            ...
                            <div class="form-group">
                                ...
                                <select name="others[]" id="others" class="form-control {{ $errors->has('others') ? 'is-invalid' : '' }}" multiple>
                                    ...
                                    @foreach ($cities as $key => $value)
                                        <option value="{{ $key }}"{{ in_array($key, old('others', [])) || $track->cities->contains($key) ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
